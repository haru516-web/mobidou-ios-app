...
export type Progress = { day: string; steps: number; dayStart: number; rewards: Reward[]; pending: string[]; routeId?: string; baseline?: number; highWater?: number; completedAt?: string };
...
export function resumeRoute(active: Progress, saved: Progress): Progress {
  ...
  return { ...saved, steps: active.steps, highWater: Math.max(steps, credit), baseline: Math.max(0, steps - credit) };
}
export function startRoute(routeId: string, steps = 0, date = new Date()): Progress {
  if (!getPilgrimage(routeId)) throw new Error('Unknown pilgrimage');
  return { ...freshProgress(date), routeId, steps, baseline: steps, highWater: steps };
}
...
export function updateSteps(progress: Progress, steps: number, date = new Date()): Progress {
  ...
  return { ...next, steps: value, rewards, pending, ...(next.routeId ? { highWater: Math.max(value, next.highWater ?? 0), ...(rewards.length === routeIds(next).length ? { completedAt: next.completedAt ?? next.day } : {}) } : {}) };
}
export function normalizeProgress(value: unknown, now = new Date()): Progress {
  ...
  if (!Number.isInteger(p.dayStart) || p.dayStart! < 0 || p.dayStart! > rewards.length || rewards.length - p.dayStart! > targets.length || !Number.isFinite(p.steps) || p.steps! < 0) throw new Error('Invalid saved day');
  ...
  return rollDay({ day: p.day, steps: p.steps!, dayStart: p.dayStart!, rewards, pending: [...new Set(pending.filter(id => rewards.some(r => r.id === id)))], ...(p.routeId ? { routeId: p.routeId, baseline: p.baseline ?? 0, highWater: p.highWater ?? p.steps, ...(rewards.length === ids.length ? { completedAt: rewards[rewards.length - 1].date } : {}) } : {}) }, now);
}
