  ...
  return <View pointerEvents="none" style={[S.roomLayer, { width: contentWidth, height: roomHeight }]}>
    {chunks.map(chunk => {
      ...
      return <View key={chunk.start} pointerEvents="none" style={[S.roomChunk, { left: chunk.start * cellWidth, width: chunk.width, height: roomHeight }]}>
        <Canvas gl={{ alpha: true }} style={{ position: 'absolute', left: 0, top: 0, width: chunk.width, height: roomHeight, backgroundColor: 'transparent' }} orthographic camera={{ left: -worldWidth / 2, right: worldWidth / 2, top: worldHeight / 2, bottom: -worldHeight / 2, position: [worldWidth / 2, 0, 10], rotation: [0, 0, 0], near: 0.1, far: 100 }} dpr={[1, 1.6]}>
