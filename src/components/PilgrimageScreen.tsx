  ...
  return <View style={{ flex: 1, backgroundColor: C.paper }}>
    ...
    <ScrollView contentContainerStyle={{ padding: 22, paddingTop: 0, paddingBottom: 40 }}>
      {preview ? <>
        ...
        <Text style={[S.body, { marginBottom: 20 }]}>森の奥へ。雲の上へ。それとも、いつもの社へ。{ '\n' }今の気持ちに合う巡礼を選んでください。</Text>
        {PILGRIMAGES.map((route, i) => <Pressable key={route.id} accessibilityRole="button" accessibilityLabel={`${route.name} ${route.type}の詳細`} onPress={() => setPreview(route)} style={S.card}>
          <Image source={PILGRIMAGE_IMAGES[route.id]} style={S.cardImage} contentFit="cover" />
          <View style={S.cardCopy}><Text style={[S.kicker, { color: route.color }]}>0{i + 1} / {route.type}{activeId === route.id ? ' · 巡礼中' : records[route.id]?.completedAt ? ' · 結願' : ''}</Text><Text style={S.cardTitle}>{route.name}</Text><Text style={S.body}>{route.subtitle}</Text><View style={S.meta}><Text style={S.small}>{route.ids.length}{route.type === '七願掛け' ? '巡' : 'か所'} · {route.targets[0].toLocaleString()}歩から</Text><Icon name="arrow-forward" size={18} color={route.color} /></View></View>
        </Pressable>)}
