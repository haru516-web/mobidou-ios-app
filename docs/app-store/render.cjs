...
const slides = [
  ...
  {id:'02-book', theme:'red', label:'歩いて、集める', title:'歩いた日々が、<br>御朱印帳になる。', sub:'もびの世界にある、42のご縁。'},
  ...
  {id:'05-mobbies', theme:'red', label:'好きな子を、相棒に', title:'18体のモビー。<br>今日は、誰と歩く？', sub:'みんな、最初から選べます。'}
];
...
const outputDir = process.env.APP_STORE_RENDER_DIR
  ? path.resolve(dir, process.env.APP_STORE_RENDER_DIR)
  : path.join(dir, 'iphone-6.9');
const artifactDir = process.env.APP_STORE_RENDER_DIR ? outputDir : dir;
const css = `
...
(async()=>{
 ...
 for(let i=0;i<slides.length;i++){
  ...
  await page.screenshot({path:path.join(outputDir,slides[i].id+'.png'),omitBackground:false});
 }
 fs.writeFileSync(path.join(artifactDir,'preview.html'),`<!doctype html><html lang="ja"><meta charset="utf-8"><title>もび道 | App Store スクリーンショット</title><style>${css}body{padding:24px;display:flex;gap:20px;flex-wrap:wrap}</style>${items.join('')}</html>`);
 const overview=await browser.newPage({viewport:{width:1320,height:616},deviceScaleFactor:1});
 await overview.setContent(`<html><style>body{margin:0;padding:20px;display:flex;gap:15px;background:#e8e1d6}img{width:244px;height:auto;border-radius:8px}</style>${slides.map(s=>`<img src="${data(path.join(outputDir,s.id+'.png'))}">`).join('')}</html>`);
 await overview.screenshot({path:path.join(artifactDir,'overview.png')});
