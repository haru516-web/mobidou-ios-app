...
- おでかけ: 1,000 / 3,000 / 5,000歩で1日最大3種。翌日は未取得の続きから。全42種でひとめぐり。
...
御朱印42枚はビルトインimage_genで生成。`mobbyyellow.PNG` をモビ神の造形参照にし、片目レンズ・十字キー・丸ボタン2つ・短い手足を維持しています。`assets/goshuin/` に1024×1536 PNGを同梱。既存6枚のプロンプトは `docs/prompt-*.txt`、追加分のルールは `docs/ARTWORK.md` に記録しています。
